"""Agent 3: personalized civic-service roadmap generation."""
...
from backend.config import PUBLIC_DATA, require_env
    ...
    if isinstance(limits, dict):
        ...
        for size, amount in limits.items():
            if isinstance(amount, (int, float)):
                formatted.append(f"Household of {size}: ${amount:,.0f}/month")
        return formatted
...
def _build_prompt(citizen: CitizenMeta | None, guide: dict[str, Any]) -> str:
    if citizen is None:
        return f"""Convert this official service documentation into a practical step-by-step roadmap for any Montgomery resident.

OFFICIAL SERVICE DOCUMENTATION
{_format_guide_for_prompt(guide)}

Requirements:
- Summarize who qualifies in eligibility_note.
- Provide a realistic total_estimated_time.
- Each step must explain what to do, which documents matter, whether it can be done online, and where to go when in-person help is needed.
- If HOW TO APPLY is empty, begin with a contact-the-provider step and then outline the safest next actions."""

    return f"""Transform this official service documentation into a personalized roadmap for the resident below.
...
Requirements:
- Explain why this resident appears eligible using their income, household, housing, or other relevant profile details.
- Rewrite each step in plain English directly for this resident.
- Filter documents down to what matters most for that resident.
- If HOW TO APPLY is empty, begin with a contact-the-provider step tailored to their situation."""


def _build_cache_key(service_id: str, citizen: CitizenMeta | None) -> str:
    if citizen is None:
        return f"roadmap:generic:{service_id}"
    citizen_hash = hashlib.md5(citizen.model_dump_json(by_alias=True).encode()).hexdigest()
