...
import { getDefaultUserLocation } from "@/lib/commuteEngine";
...
interface CommuteMapProps {
  ...
  selectedCoords: { lat: number; lng: number } | undefined | null;
    ...
    <div className="h-[45%] shrink-0 relative">
        ...
        {jobListings
          ...
          .map((job) => (
            <Marker
              ...
              eventHandlers={{ click: () => onJobMarkerClick(job.id) }}
            >
              <Popup>
      ...
      <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 shadow-sm z-[1000] flex items-center gap-4">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-muted-foreground">{transitRouteCount} bus routes</span>
        </div>
      </div>
    </div>
  );
